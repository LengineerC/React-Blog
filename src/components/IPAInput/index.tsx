...
const keyAreas = [
  {
    title: '辅音',
    symbols: ['p', 'b', 't', 'd', 'ʈ', 'ɖ', 'c', 'ɟ', 'k', 'ɡ', 'q', 'ɢ', 'm', 'ɱ', 'ʙ', 'n', 'r', 'ɿ', 'ɾ', 'ɳ', 'ɽ', 'ɲ', 'ȵ', 'ŋ', 'ɴ', 'ʀ', 'ʔ', 'ʡ', 'ʢ', 'ɸ', 'f', 'θ', 'ɬ', 's', 'ʃ', 'ʅ', 'ɧ', 'ʆ', 'ʂ', 'ç', 'ɕ', 'x', 'χ', 'ħ', 'ʜ', 'h', 'β', 'v', 'ⱱ', 'ð', 'ɮ', 'z', 'ʒ', 'ʓ', 'ʐ', 'ʝ', 'ʑ', 'ɣ', 'ʁ', 'ʕ', 'ɦ', 'w', 'ʍ', 'ʋ', 'ɹ', 'ɺ', 'l', 'ɫ', 'ȴ', 'ɻ', 'ɭ', 'ɥ', 'j', 'ʎ', 'ɰ', 'ʟ', 'ɓ', 'ɗ', 'ᶑ', 'ʄ', 'ɠ', 'ʛ', 'ʠ', 'ʘ', 'ǀ', 'ǃ', '¡', 'ǂ', 'ʞ']
  },
  {
    title: '元音',
    symbols: ['i', 'y', 'ɨ', 'ʉ', 'ɯ', 'u', 'ɪ', 'ʏ', 'ʊ', 'e', 'ø', 'ɘ', 'ɵ', 'ə', 'ɚ', 'ɤ', 'o', 'ɛ', 'œ', 'ɜ', 'ɝ', 'ɞ', 'ɐ', 'ʌ', 'ɔ', 'æ', 'a', 'ɶ', 'ɑ', 'ɒ']
  },
  {
    title: '连接符',
    symbols: ['͡', '͜']
  ...
  {
    title: '声调',
    symbols: ['˥', '˦', '˧', '˨', '˩', '̋', '́', '̄', '̀', '̏', '↗', '̌', 'ꜛ', '↘', '̂', 'ꜜ', '᷄', '᷅', '→', '᷈']
  },
  {
    title: '发音修饰',
    symbols: ['̝', '˔', '̞', '˕', '̹', '͗', '˒', '̜', '͑', '˓', '̟', '˗', '˖', '̠', 'ⁿ', 'ˡ', '̚', 'ᵊ', 'ᶿ', 'ʰ', 'ˣ', 'ʼ', '̩', '̍', '̯', '̑', '̘', '̙', '̈', '̽', '̪', '̻', '͆', '̺', 'ʷ', '̼', 'ʲ', 'ˠ', 'ˁ', 'ˀ', '̴', '̥', '̊', '̬', '̤', '̰', '̃', '˞', '̆']
  },
  {
    title: '元符号',
    symbols: ['⸨', '⸩', '(', ')', '[', ']', '/', '{', '}', '⟨', '‹', '⟩', '›', '<', '>',]
...
export default function IPAInput() {
  ...
  const createAreas = () => keyAreas.map((areaData, i) => (
    <React.Fragment key={`area-${i}`}>
      <div className={`keys-area ${darkMode && 'dark'}`}>
        <div className='keys-area-title'>
          {areaData.title}
        </div>
    ...
    <div className='ipa-main'>
      ...
      >
        ...
        <div className={`tool-bar ${showToolbar && 'fade-in'}`}>
          <CopyFilled
            className={`tool-btn ${darkMode && "dark"}`}
            onClick={handleCopy}
          />
