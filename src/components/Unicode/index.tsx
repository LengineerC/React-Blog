    ...
    <div className="unicode-main">
      ...
      <div className="center">
        <div className="convert-switcher">
          ...
          <div className={`switcher-btn ${darkMode && 'dark'}`} onClick={toggleConvertMode}>
            <FontAwesomeIcon icon={faRepeat} color={darkMode ? '#ffffffcc' : '#001447'} size="xl" />
          </div>
        ...
        <div className={`convert-btn ${darkMode && 'dark'}`} onClick={handleConvert}>
          转换
        </div>
      </div>
      ...
      <div className="textarea-container">
        ...
        >
          <div className={`label ${darkMode && 'dark'}`}>转换结果:</div>

          <CopyFilled className={`copy-btn ${darkMode && 'dark'}`} onClick={copyResult} />
        </div>

        <textarea className={`${darkMode && 'dark'}`} value={resultText} disabled />
      </div>
    </div>
  );
}
