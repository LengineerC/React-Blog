...
export default function PostsInfoCard() {
  ...
  const postsCount = useAppSelector(state => state.post.postList).length;
  const tagsCount = Object.keys(useAppSelector(state => state.taxonomy.tagsList)).length;
  const categoriesCount = Object.keys(
    useAppSelector(state => state.taxonomy.categoriesList),
  ).length;
    ...
    <Card className="aside-card" scale={true} darkMode={darkMode}>
      <div className="post-info-card-main">
        <div className="post-info-card-col-1">
          <div
            className={darkMode ? 'post-info-card-col-header-dark' : 'post-info-card-col-header'}
          >
            <FontAwesomeIcon icon={faBook} /> 文章
          </div>
          <div
            className={darkMode ? 'post-info-card-col-content-dark' : 'post-info-card-col-content'}
          >
            {/* <NavLink to="/posts" > */}
            {postsCount}
            {/* </NavLink> */}
        ...
        <div className="post-info-card-col-2">
          ...
          <div
            className={darkMode ? 'post-info-card-col-content-dark' : 'post-info-card-col-content'}
          >
            <NavLink to="/tags">{tagsCount}</NavLink>
          </div>
        </div>
        ...
        <div className="post-info-card-col-3">
          ...
          <div
            className={darkMode ? 'post-info-card-col-content-dark' : 'post-info-card-col-content'}
          >
            <NavLink to="/categories">{categoriesCount}</NavLink>
          </div>
        </div>
      </div>
    </Card>
